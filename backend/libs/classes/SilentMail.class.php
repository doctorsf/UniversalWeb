...
class SilentMail {
    ...
    public function send($trace='off') {
		if ($this->_from === null) {
			$this->_setErreur(-1, getLib('PAS_EXPEDITEUR'));
			return $this->getErreurNum();
        }
		elseif (!(mb_eregi("^[_\.0-9a-z-]+@([0-9a-z-]+\.)+[a-z]{2,4}$", $this->_from))) {
			$this->_setErreur(-2, getLib('EMAIL_EXPEDITEUR_KO'));
			return $this->getErreurNum();
		}
        elseif (count($this->_to) === 0) {
			$this->_setErreur(-4, getLib('PAS_DESTINATAIRE'));
			return $this->getErreurNum();
        }
        elseif (count($this->_to) > 3) {
			$this->_setErreur(-5, getLib('TROP_DESTINATAIRES'));
			return $this->getErreurNum();
		}
        elseif ($this->_subject === null) {
			$this->_setErreur(-3, getLib('PAS_OBJET'));
			return $this->getErreurNum();
        }        
		...
		foreach($this->_to as $dest) {
			...
			$this->_setErreur(-6, getLib('EMAIL_DESTINATAIRE_INVALIDE'));
				return $this->getErreurNum();
			}
		}
        ...
        if (!$boSend) {
			...
			$this->_setErreur(-7, getLib('EMAIL_ENVOI_ERREUR'));
			return $this->getErreurNum();
        }        
